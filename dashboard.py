...
def extract_country_details(soup):
    """Best-effort extraction of a game's store listing details for one country.
    Note: og:image/description/screenshot detection are heuristic — Play Store's
    exact markup varies, so verify against a couple of real games and tell me
    if any field consistently comes back wrong."""
    if not soup:
        return None

    details = {"title": None, "icon": None, "feature_graphic": None,
               "screenshots": [], "description": None, "category": None,
               "status": "unavailable"}

    page_text = soup.get_text(" ", strip=True)

    if re.search(r"pre-?register", page_text, re.IGNORECASE):
        details["status"] = "pre_register"
    elif soup.title:
        details["status"] = "live"

    details["title"] = extract_canonical_title(soup)

    og_image = soup.find("meta", property="og:image")
    if og_image and og_image.get("content"):
        details["feature_graphic"] = og_image["content"]
    ...
    if icon_img and icon_img.get("src"):
        details["icon"] = icon_img["src"]
    elif not details["icon"]:
        first_googleusercontent_img = soup.find(
            "img", src=lambda s: s and "googleusercontent.com" in s
        )
        if first_googleusercontent_img:
            details["icon"] = first_googleusercontent_img.get("src")

    screenshot_imgs = soup.find_all(
        "img", src=lambda s: s and "googleusercontent.com" in s
    )
    seen_srcs = set()
    for img in screenshot_imgs:
        src = img.get("src")
        if src and src not in seen_srcs and src != details.get("icon"):
            seen_srcs.add(src)
            details["screenshots"].append(src)
        if len(details["screenshots"]) >= 6:
            break

    for a in soup.find_all("a", href=True):
        if "/store/apps/category/" in a["href"]:
            details["category"] = a.get_text(strip=True)
            break

    return details


def check_country_status(package_name, country_code):
    """Lightweight check for the Live/Pre-register/Unavailable scan across all countries."""
    soup = fetch_app_page(package_name, country=country_code.lower())
    if not soup:
        return "unavailable"
    page_text = soup.get_text(" ", strip=True)
    if re.search(r"pre-?register", page_text, re.IGNORECASE):
...
with tab_country:
    ...
    st.caption("Compare a game's store listing between two countries, then scan all countries for Live / Pre-register / Unavailable status.")

    country_url = st.text_input("Paste the Play Store game URL", key="country_url_input")

    country_names_sorted = sorted(PLAY_STORE_COUNTRIES.items(), key=lambda x: x[1])
    country_labels = [f"{name} ({code})" for code, name in country_names_sorted]
    ...
    if st.button("🌍 Load Comparison", type="primary"):
        ...
        if not package_name:
            ...
            country_a_code = code_by_label[country_a_label]
            country_b_code = code_by_label[country_b_label]

            with st.spinner(f"Loading listing for {country_a_label} and {country_b_label}..."):
                soup_a = fetch_app_page(package_name, country=country_a_code.lower())
                soup_b = fetch_app_page(package_name, country=country_b_code.lower())
                details_a = extract_country_details(soup_a)
                details_b = extract_country_details(soup_b)

            st.session_state.country_compare = {
                "package_name": package_name,
                "a_label": country_a_label, "b_label": country_b_label,
                "details_a": details_a, "details_b": details_b,
            }
            st.session_state.country_full_scan = None  # reset any previous scan

    if st.session_state.get("country_compare"):
        cmp = st.session_state.country_compare
        st.markdown("---")
        st.markdown(f"### Comparison for `{cmp['package_name']}`")

        col_a, col_b = st.columns(2)
        for col, label, details in [(col_a, cmp["a_label"], cmp["details_a"]), (col_b, cmp["b_label"], cmp["details_b"])]:
            with col:
                st.markdown(f"#### {label}")
                if not details or details["status"] == "unavailable":
                    st.error("Unavailable in this country")
                else:
                    status_label = "🟢 Live" if details["status"] == "live" else "🟡 Pre-register"
                    st.write(f"**Status:** {status_label}")
                    st.write(f"**Title:** {details.get('title') or '-'}")
                    st.write(f"**Category:** {details.get('category') or '-'}")
                    if details.get("icon"):
                        st.image(details["icon"], width=80, caption="Icon")
                    if details.get("feature_graphic"):
                        st.image(details["feature_graphic"], caption="Feature Graphic")
                    if details.get("description"):
                        st.write(f"**Description:** {details['description'][:400]}{'...' if len(details['description']) > 400 else ''}")
                    if details.get("screenshots"):
                        st.write("**Screenshots:**")
                        ss_cols = st.columns(min(3, len(details["screenshots"])))
                        for i, ss in enumerate(details["screenshots"][:6]):
                            ss_cols[i % len(ss_cols)].image(ss, use_container_width=True)

        st.markdown("---")
        st.write("Ready to scan all countries for availability status? This checks all 177 countries and will take a while.")

        if st.button("🌍 Scan All Countries", type="primary"):
            progress = st.progress(0)
            status_text = st.empty()
            results = {"live": [], "pre_register": [], "unavailable": []}
            codes = list(PLAY_STORE_COUNTRIES.keys())

            for i, code in enumerate(codes):
                status_text.write(f"Checking {PLAY_STORE_COUNTRIES[code]} ({code})... [{i+1}/{len(codes)}]")
                status = check_country_status(cmp["package_name"], code)
                results[status].append(f"{PLAY_STORE_COUNTRIES[code]} ({code})")
                progress.progress((i + 1) / len(codes))

            status_text.empty()
            st.session_state.country_full_scan = results

    if st.session_state.get("country_full_scan"):
        scan = st.session_state.country_full_scan
        st.markdown("---")
        st.markdown("### Full Country Scan Results")

        scol1, scol2, scol3 = st.columns(3)
        with scol1:
            st.markdown(f"**🟢 Live ({len(scan['live'])})**")
            for c in scan["live"]:
                st.write(c)
        with scol2:
            st.markdown(f"**🟡 Pre-register ({len(scan['pre_register'])})**")
            for c in scan["pre_register"]:
                st.write(c)
        with scol3:
            st.markdown(f"**🔴 Unavailable ({len(scan['unavailable'])})**")
            for c in scan["unavailable"]:
                st.write(c)
